...
export class UsersController {
  ...
  async getUserById(req, res) {
    try {
      const userIdFromJWT = req.user.id; // Ambil user_id dari token JWT yang sudah disisipkan di req.user
      const sqlDataGet = "SELECT * FROM users WHERE user_id = ?";

      connection.query(sqlDataGet, [userIdFromJWT], (err, result) => {
        if (err) {
          return res
            .status(500)
        ...
        // Sanitasi data dengan menghapus password
        const sanitizedData = result.map(({ password, ...user }) => user);
